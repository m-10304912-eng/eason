...
<body class="bg-white flex items-center justify-center min-h-screen p-4">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-gray-100 hover-card transition-all duration-300">
        ...
        if (isset($_POST['register'])) {
            ...
            if (empty($nokp) || empty($nama) || empty($password)) {
                echo "<script>Swal.fire('Ralat!', 'Sila isikan ruang ini.', 'error');</script>";
            } else if (!ctype_digit($nokp)) {
                echo "<script>Swal.fire('Ralat!', 'Jenis data: Sila masukkan nombor sahaja.', 'error');</script>";
            } else if (strlen($nokp) < 12) {
                echo "<script>Swal.fire('Ralat!', 'Had bawah: sila masukkan 12 angka.', 'error');</script>";
            } else if (strlen($nokp) > 12) {
                echo "<script>Swal.fire('Ralat!', 'Had atas: sila masukkan 12 angka sahaja.', 'error');</script>";
            } else {
                // Check duplicate
        ...
        <form method="post" class="space-y-5">
            <div>
                ...
                <input type="text" name="nokp" required pattern="[0-9]+" minlength="12" maxlength="12" title="Jenis data: Sila masukkan nombor sahaja. Had bawah: sila masukkan 12 angka. Had atas: sila masukkan 12 angka sahaja." class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 transition-shadow" placeholder="Contoh: 091023070189">
            ...
            <div>
                ...
                <input type="text" name="nama" required title="Sila isikan ruang ini" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 transition-shadow" placeholder="Nama Penuh">
            </div>
